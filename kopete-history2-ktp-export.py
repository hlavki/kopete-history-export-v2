...
def export(accountId, dbPath, exportDir):
    ...
    cursor = conn.cursor()
    filesSql = """SELECT h1.other_id, strftime('%%Y-%%m-%%d', h1.datetime) AS day 
                    FROM history h1 
    ...
    for row in rows:
        ...
        messagesSql = """SELECT id, protocol, account, direction, me_id, me_nick, other_id, other_nick, datetime, message 
                            FROM history 
                            WHERE other_id = '%s' AND strftime('%%Y-%%m-%%d', datetime) = '%s' 
                            ORDER BY datetime ASC""" % (otherId, "-".join(day))
        for brow in cursor.execute(messagesSql):
            msgEl = ET.SubElement(rootEl, "message")
            msgTime = dateutil.parser.parse(brow[8]);
            msgEl.set("time", msgTime.strftime('%Y%m%dT%H:%M:%S'))
            msgEl.set("id", brow[6] if brow[3] == "1" else brow[4])
            msgEl.set("name", brow[7] if brow[3] == "1" else brow[5])
            msgEl.set("issuer", "true" if brow[3] == "0" else "false")
            msgEl.set("token", "")
            msgEl.set("message-token", "")
            msgEl.set("type", "normal")
            msgEl.text = brow[9]
            indent(rootEl)

        tree = ET.ElementTree(rootEl)
        contactDirectory = os.path.join(exportDir, otherId);
        if not os.path.exists(contactDirectory): # create directory if doesn't exists
            os.makedirs(contactDirectory)

        exportFileName = day[0] + day[1] + day[2] + ".log";
        with open(os.path.join(contactDirectory, exportFileName), 'w') as output:
